...
backward=()=>{
    ...
    switch (n) {
        case 1:
            fx = (this.Solve(eq,x)) - this.Solve(eq,x-(1*h)) / h
            console.log(fx);
            break;
        case 2:
            fx = (this.Solve(eq,x)) - 2*this.Solve(eq,x-(1*h)) + this.Solve(eq,x+(2*h)) / pow(h, 2)
            break;
        case 3:
            fx = (this.Solve(eq,x)) - 3*this.Solve(eq,x+(1*h)) + 3*this.Solve(eq,x+(2*h)) - this.Solve(eq,x-(3*h))/ pow(h, 3)
            break;
        default:
            fx = (this.Solve(eq,x)) - 4*this.Solve(eq,x+(1*h)) + 6*this.Solve(eq,x+(2*h)) - 4*this.Solve(eq,x+(3*h)) + this.Solve(eq,x-(4*h)) /pow(h, 4) 
    }
        ...
        <Layout className="site-layout">
          ...
          <Content style={{ margin: '0 16px' }}>
            <Breadcrumb style={{ margin: '16px 0' }}>
              ...
              <Breadcrumb.Item>Backward O(h)</Breadcrumb.Item>
            </Breadcrumb>
            ...
            BACKWARD O(h)<br/><br/>
              ...
              <div>
                <Button type="primary" onClick={this.backward} >Submit</Button>
              </div>
              <br />
