...
componentDidMount()
    {
        axios.get('http://localhost/numerical/php/central.php')
        .then(res=>{
...
central=()=>{
    ...
    switch (n) {
        case 1:
            fx = (this.Solve(eq,x+(1*h)) - this.Solve(eq,x-(1*h))) / 2*h
            console.log(fx);
            break;
        case 2:
            fx = (this.Solve(eq,x+(1*h)) - 2*this.Solve(eq,x) + this.Solve(eq,x-(1*h))) / pow(h, 2)
            break;
        case 3:
            fx = (this.Solve(eq,x+(2*h)) - 2*this.Solve(eq,x+(1*h)) + 2*this.Solve(eq,x-(1*h)) - this.Solve(eq,x-(2*h))) / 2*(pow(h, 3))
            break;
        default:
            fx = (this.Solve(eq,x+(2*h)) - 4*this.Solve(eq,x+(1*h)) + 6*this.Solve(eq,x) - 4*this.Solve(eq,x-(1*h)) + this.Solve(eq,x-(2*h))) /pow(h, 4) 
    }
        ...
        <Layout className="site-layout">
          ...
          <Content style={{ margin: '0 16px' }}>
            <Breadcrumb style={{ margin: '16px 0' }}>
              ...
              <Breadcrumb.Item>Central O(h)</Breadcrumb.Item>
            </Breadcrumb>
            ...
            <div className="site-layout-background" style={{ padding: 24, minHeight: 360 }}>
              ...
              <div>
                <Button type="primary" onClick={this.central}>Submit</Button>
              </div>
              <br />
