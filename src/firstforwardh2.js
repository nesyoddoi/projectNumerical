...
class Firstforward extends Component {
  state = {
    collapsed: false,  
  };
  // handleSizeChange = e => {
...
componentDidMount()
    {
        axios.get('http://localhost/numerical/php/firstforward.php')
        .then(res=>{
...
firstforward=()=>{
    ...
    switch (n) {
        case 1:
            fx = (-this.Solve(x+(2*h)) + 4*this.Solve(x+(1*h)) - 3*this.Solve(x)) / (2*h)
            console.log(fx);
            break;
        case 2:
            fx = (-this.Solve(x+(3*h)) + 4*this.Solve(x+(2*h)) - 5*this.Solve(x+(1*h)) + 2*this.Solve(x)) / pow(h, 2)
            break;
        case 3:
            fx = (-3*this.Solve(x+(4*h)) + 14*this.Solve(x+(3*h)) - 24*this.Solve(x+(2*h)) + 18*this.Solve(x+(1*h)) - 5*this.Solve(x)) / 2*(pow(h, 3))
            break;
        default:
            fx = (-2*this.Solve(x+(5*h)) + 11*this.Solve(x+(4*h)) - 24*this.Solve(x+(3*h)) + 26*this.Solve(x+(2*h)) - 14*this.Solve(x+(1*h)) + 3*this.Solve(x)) / pow(h, 4) 
    }
        ...
        <Layout className="site-layout">
          ...
          <Content style={{ margin: '0 16px' }}>
            <Breadcrumb style={{ margin: '16px 0' }}>
              ...
              <Breadcrumb.Item>First Forward O(h^2)</Breadcrumb.Item>
            </Breadcrumb>
            
            <div className="site-layout-background" style={{ padding: 24, minHeight: 360 }}>
            FIRST FORWARD O(h^2)<br/><br/>
              <div>
              ...
              <div>
                <Button type="primary" onClick={this.firstforward}>Submit</Button>
              </div>
              <br />
