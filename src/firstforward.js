...
firstforward=()=>{
    ...
    switch (n) {
        case 1:
            fx = (this.Solve(eq,x+(1*h)) - this.Solve(eq,x)) / h
            console.log(fx);
            break;
        case 2:
            fx = (this.Solve(eq,x+(2*h)) - 2*this.Solve(eq,x+(1*h)) + this.Solve(eq,x)) / pow(h, 2)
            break;
        case 3:
            fx = (this.Solve(eq,x+(3*h)) - 3*this.Solve(eq,x+(2*h)) + 3*this.Solve(eq,x+(1*h)) - this.Solve(eq,x)) / pow(h, 3)
            break;
        default:
            fx = (this.Solve(eq,x+(4*h)) - 4*this.Solve(eq,x+(3*h)) + 6*this.Solve(eq,x+(2*h)) - 4*this.Solve(eq,x+(1*h)) + this.Solve(eq,x)) /pow(h, 4) 
    }
        ...
        <Layout className="site-layout">
          ...
          <Content style={{ margin: '0 16px' }}>
            <Breadcrumb style={{ margin: '16px 0' }}>
              ...
              <Breadcrumb.Item>First Forward O(h)</Breadcrumb.Item>
            </Breadcrumb>
            
            <div className="site-layout-background" style={{ padding: 24, minHeight: 360 }}>
            FIRST FORWARD O(h)<br/><br/>
              <div>
